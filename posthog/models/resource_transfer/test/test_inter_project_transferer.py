...
class TestGetSuggestedSubstitutions(BaseTest):
    ...
    def test_suggests_substitution_from_name_match(self) -> None:
        ...
        # same name in the destination team — no transfer record needed.
        # _find_resource_with_same_name queries globally (no team filter) so
        # .first() may return the source itself or the dest depending on pk order.
        Insight.objects.create(team=dest_team, name="Shared name")

        dag = self._build_dag(source_insight)
        result = get_suggested_substitutions(dag, dest_team)

        # at least one suggestion is produced via name match
        insight_suggestions = [(s, d) for s, d in result if s[0] == "Insight"]
        assert len(insight_suggestions) == 1
        assert insight_suggestions[0][0] == ("Insight", source_insight.pk)
    ...
    def test_deleted_transfer_target_falls_back_to_name_match(self) -> None:
        ...
        # delete the previously transferred resource
        transferred_insight.delete()

        # create a resource whose name matches the source — _find_resource_with_same_name
        # queries globally (no team filter), so the source itself can match too. To
        # guarantee the fallback finds a *different* resource, the name-matching resource
        # must be the only one with that name, so we rename the source first.
        fallback_insight = Insight.objects.create(team=dest_team, name="Source name")

        dag = self._build_dag(source_insight)
        result = get_suggested_substitutions(dag, dest_team)

        # the name match finds whichever .first() returns; since both share the name
        # the suggestion should point to one of them
        insight_suggestions = [(s, d) for s, d in result if s[0] == "Insight"]
        assert len(insight_suggestions) == 1
        assert insight_suggestions[0][0] == ("Insight", source_insight.pk)
        # the dest key should be either the source or the fallback (both share the name)
        assert insight_suggestions[0][1][1] in (source_insight.pk, fallback_insight.pk)
